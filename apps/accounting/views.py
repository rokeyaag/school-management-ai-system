...
def summary(request):
    ...
    fee_income = FeePayment.objects.filter(school=school, status='paid', paid_date__month=month, paid_date__year=year)
    total_expense = float(sum(e.amount for e in expenses))
    total_income = float(sum(i.amount for i in incomes))
    total_fee = float(sum(f.paid_amount or 0 for f in fee_income))
    total_income_all = total_income + total_fee
    return Response({
        'month': month,
        'year': year,
        ...
        'profit': total_income_all - total_expense,
